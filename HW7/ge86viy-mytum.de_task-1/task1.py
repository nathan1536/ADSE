...
def get_aabb_polygon(trajectory: dict):
    ...
    #  Start of your code  #
    ########################
    Pol = Polygon(trajectory["xy"])
    aabb = unary_union(Pol).envelope
    ########################
    #   End of your code   #
    ########################
...
def calculate_mbc_radius(rectangle_width: float, rectangle_length: float):
    ...
    #  Start of your code  #
    ########################
    radius = 1 / 2 * (np.sqrt(rectangle_width ** 2 + rectangle_length ** 2))
    radius = float(radius)
    ########################
    #   End of your code   #
    ########################
...
def check_circle_collision(xy1: np.ndarray, xy2: np.ndarray, r: float):
    ...
    #  Start of your code  #
    ########################
    dx = np.abs(xy1[0] - xy2[0])
    dy = np.abs(xy1[1] - xy2[1])
...
def polygon_collision_check(trajectory1: dict, trajectory2: dict):
    ...
    #  Start of your code  #
    ########################
    # dx = np.abs(trajectory1.xy[ :, 0] - trajectory2.xy[ :, 0])
    # dy = np.abs(trajectory1.xy[ :, 1] - trajectory2.xy[ :, 1])
    # distance = np.sqrt(dx * dx + dy * dy)
    for i in range(len(trajectory1['x'])):
        for j in range(len(trajectory2['x'])):
            point1 = po(traj1['x'][i], traj1['y'][i]).buffer(traj1['radius'][i])
            point2 = Point(traj2['x'][j], traj2['y'][j]).buffer(traj2['radius'][j])
...
def mbc_collision_check(trajectory1: dict, trajectory2: dict, mbc_radius: float,
    ...
    #  Start of your code  #
    ########################
    t1_xy = trajectory1['xy']
    t2_xy = trajectory2['xy']
    mbc_collision = np.any(given_circle_collision_check(t1_xy, t2_xy, mbc_radius))
    ########################
    #   End of your code   #
    ########################
