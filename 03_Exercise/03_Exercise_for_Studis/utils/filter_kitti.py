...
def plot(img, boxes):
    ...
    plt.imshow(img[:, :, ::-1])
    rects = [Rectangle((box[0], box[1]), box[2] - box[0], box[3] - box[1], linewidth=1, edgecolor='r', facecolor='none') for box in boxes]
    for rect in rects:
        ax.add_patch(rect)
...
def crop(img, boxes, classes):
    ...
    mask = np.expand_dims((mask_x & mask_y), -1)  # [n, 1], mask for the boxes within the image after crop.
...
for label_idx, label_file in enumerate(labels_list):
    ...
    with open(os.path.join(label_path, label_file)) as f:
        ...
        for line_idx, line in enumerate(f):
            ...
            if obj_class == "Car":
                new_class = 0  # Car
                new_labels = new_labels + str(new_class) + " " + str(x) + " " + str(y) + " " + str(width) + " " + str(height) + "\n"
    ...
    for b, box in enumerate(boxes_list):
        labels_write = labels_write + str(int(classes[b])) + " " + str(box[0]) + " " + str(box[1]) + " " \
                       + str(box[2]) + " " + str(box[3]) + "\n"
        if int(classes[b]) == 0:
            car_count += 1
        if int(classes[b]) == 1:
