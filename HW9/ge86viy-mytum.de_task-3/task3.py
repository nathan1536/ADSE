...
def check_trajectory(ttc, a_comb, bound_collision, ttc_treshold=2.0, a_comb_threshold=15.0):
    ...
    #  Start of your code  #
    ########################
    if ttc <= ttc_treshold or a_comb.max() > a_comb_threshold or bound_collision is True:
        safe = False
    else:
        safe = True
    ########################
...
def check_bound_collision(path, bound, veh_width=2.8):
    ...
    x = shapely.geometry.LineString(path).buffer(veh_width / 2, 1)
    y = shapely.geometry.LineString(bound)
    a = x.intersection(y)
    print(a)
    if a is not None:
        intersection = True
